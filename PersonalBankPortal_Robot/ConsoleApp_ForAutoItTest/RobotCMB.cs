...
namespace ConsoleApp_ForAutoItTest
{
    public class RobotCMB
    {
        ...
        private void FillInterBankTransInfo(RobotContext context)
        {
            ...
            thread = new Thread(() => Clipboard.SetText(context.WithdrawAmount));
            thread.SetApartmentState(ApartmentState.STA); //Set the thread to STA
            thread.Start();
            thread.Join();
            ...
            thread = new Thread(() => Clipboard.SetText(context.BoTransactionId));
            thread.SetApartmentState(ApartmentState.STA); //Set the thread to STA
            thread.Start();
            thread.Join();

            inputSimulator.Mouse.MoveMouseTo(14000, 43000); //Transfer Reference
            Thread.Sleep(GetRandomNumber(100));
            inputSimulator.Mouse.LeftButtonClick();
        ...
        private void FillSameBankTransInfo(RobotContext context)
        {
            ...
            if (!string.IsNullOrEmpty(context.ToAccountCity))
            {
                //City Name COPY
                thread = new Thread(() => Clipboard.SetText(context.ToAccountCity));
                thread.SetApartmentState(ApartmentState.STA); //Set the thread to STA
                thread.Start();
                thread.Join();

                inputSimulator.Mouse.MoveMouseTo(14000, 31000); //City Name
                Thread.Sleep(GetRandomNumber(100));
                inputSimulator.Mouse.LeftButtonClick();
                Thread.Sleep(GetRandomNumber(200));


                inputSimulator.Mouse.MoveMouseTo(14000, 33000); //city Search
                Thread.Sleep(GetRandomNumber(100));
                inputSimulator.Mouse.LeftButtonClick();
                Thread.Sleep(GetRandomNumber(200));
                ...
                inputSimulator.Mouse.MoveMouseTo(14000, 35000); //city Select
                Thread.Sleep(GetRandomNumber(100));
                inputSimulator.Mouse.LeftButtonClick();
                Thread.Sleep(GetRandomNumber(100));
            }
            ...
            inputSimulator.Mouse.MoveMouseTo(14000, 39000); //Transfer Reference
            Thread.Sleep(GetRandomNumber(100));
            inputSimulator.Mouse.LeftButtonClick();
            ...
            for (int j = 0; j < 5; j++)
            {
                var popup = SimulateKey.SetForegroundWindow(SimulateKey.FindWindow("TPbBaseMsgForm", null));
                if (popup)
                {
                    Thread.Sleep(GetRandomNumber(1000));
        ...
        private void FillTransferPassword(RobotContext context)
        {
            ...
            SimulateKey.SendText(y,context.TokenWithdrawPin);
            ...
            for (int j = 0; j < 10; j++)
            {
                var popup = SimulateKey.SetForegroundWindow(SimulateKey.FindWindow("TTransferSuccessFrm", null));
                if (popup)
                {
                    //Thread.Sleep(GetRandomNumber(1000));
                    //inputSimulator.Mouse.MoveMouseTo(31000, 36500); //Transfer Okay Confirm
        ...
        private RobotResult DoLogOut(RobotContext context)
        {
            try
            {
                ...
                inputSimulator.Mouse.MoveMouseTo(57000, 6000); //Logout Button
                Thread.Sleep(GetRandomNumber(100));
                inputSimulator.Mouse.LeftButtonClick();
                Thread.Sleep(1000);

                inputSimulator.Mouse.MoveMouseTo(30000, 37000); //Logout Button
                Thread.Sleep(GetRandomNumber(100));
                inputSimulator.Mouse.LeftButtonClick();
                Thread.Sleep(1000);
